/*
 * STM32F7xx SoC initialization for Pebble smartwatch (Robert board)
 *
 * Ported from QEMU 2.5.0-pebble8 stm32f7xx.c to QEMU 10.x APIs.
...
/* SYSCLK frequency (200MHz for STM32F7xx) */
            ...
            ARMCPU **cpu)
{
    ...
    memory_region_init_ram(flash, NULL, "stm32f7xx.flash",
                           flash_size * 1024, &err);
    if (err) {
    ...
    memory_region_init_alias(flash_alias, NULL, "stm32f7xx.flash.alias",
                             flash, 0, flash_size * 1024);
    memory_region_add_subregion(system_memory, 0, flash_alias);

    /* SRAM at 0x20000000 */
    MemoryRegion *sram = g_new(MemoryRegion, 1);
    memory_region_init_ram(sram, NULL, "stm32f7xx.sram",
                           ram_size * 1024, &err);
    if (err) {
        error_report_err(err);
        exit(1);
    ...
    memory_region_init_ram(ccm, NULL, "stm32f7xx.ccm", 64 * 1024, &err);
    if (err) {
        error_report_err(err);
        exit(1);
    }
    ...
    *cpu = ARM_CPU(first_cpu);
    ...
    /* === UARTs ===
     * STM32F7xx UART base addresses (from ST reference manual RM0383):
     *   USART1: 0x40011000  USART2: 0x40004000  USART3: 0x40004C00
     *   UART4:  0x40004C00  UART5:  0x40005000  USART6: 0x40010000
     *   UART7:  0x40007800  UART8:  0x40007C00
     * F7 USART2=0x40004000 differs from F4=0x40004400
     * F7 USART3=0x40004C00 differs from F4=0x40004800
     * F7 USART6=0x40010000 differs from F4=0x40011400
     * All other addresses appear identical.
     */
    struct {
        uint32_t addr;
        uint8_t irq_idx;
    } const uart_desc[] = {
        {0x40011000, STM32_UART1_IRQ},   /* USART1 - same as F4 */
        {0x40004000, STM32_UART2_IRQ},   /* USART2 - F7 differs from F4=0x40004400 */
        {0x40004C00, STM32_UART3_IRQ},   /* USART3 - F7 differs from F4=0x40004800 */
        {0x40004C00, STM32_UART4_IRQ},   /* UART4 - same as F4 */
        {0x40005000, STM32_UART5_IRQ},   /* UART5 - same as F4 */
        {0x40010000, STM32_UART6_IRQ},   /* USART6 - F7 differs from F4=0x40011400 */
        {0x40007800, 0},                   /* UART7 - same as F4 */
        {0x40007C00, 0},                   /* UART8 - same as F4 */
    };
    ...
    *stm32_rtc = rtc_dev;
    ...
    /* === DMA === */
    /* Note: DMA stream IRQs are NOT contiguous on STM32F7xx.
    ...
    /* === External SDRAM at 0xC0000000 (8MB for Emery framebuffer) === */
    {
        ...
        memory_region_init_ram(sdram, NULL, "stm32f7xx.sdram",
